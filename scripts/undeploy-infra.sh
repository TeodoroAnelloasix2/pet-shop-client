...
Delete_images(){
    ...
    image_list=$(aws ecr list-images --repository-name "$ecr_repository"  --region "$aws_region" --no-cli-pager  --output json  --query 'imageIds[*].imageDigest')
    img_num=$(echo "$image_list" | jq '.[] | length')
    image_ids=$(echo "$image_digests" | jq '[.[] | {imageDigest: .}]')
    if [[ ! -z "ecr_arn" ]];then 
