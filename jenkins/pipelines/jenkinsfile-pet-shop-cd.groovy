    ...
    stages{
        ...
        stage('kustomize image'){
            steps{
                ...
                sh('''
                #!/bin/bash
                last_tag=$(aws ecr list-images --repository-name prod_petshop_project --filter tagStatus=TAGGED,imageStatus=ACTIVE   --query "sort_by(imageIds, &imageTag)[-1].imageTag"  --output text)
